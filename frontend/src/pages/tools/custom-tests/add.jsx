...
All UPNs: {{join(Result[*].UserPrincipalName, ", ")}}`,
  ...
  const handleScriptEditorMount = (_editor, monaco) => {
    ...
    const cacheProvider = monaco.languages.registerCompletionItemProvider('powershell', {
      triggerCharacters: ["'", '"', '.'],
      provideCompletionItems: (model, position) => {
        ...
        if (typeMatch) {
          ...
          return {
            suggestions: shape.map((entry) => ({
              label: entry.count != null ? `${entry.type} (${entry.count})` : entry.type,
        ...
        const accessMatch = linePrefix.match(/\$[A-Za-z_][A-Za-z0-9_]*((?:\.[A-Za-z0-9_]*)+)$/)
        if (!accessMatch) {
          return { suggestions: [] }
        }

        const referenced = new Set()
        const typeRegex = /Get-CIPPTestData[^\r\n]*?-Type\s*['"]([A-Za-z0-9_]+)['"]/gi
        let typeHit
        ...
        shape
          ...
          .forEach((entry) => {
            entry.fields.forEach((field) => {
              if (!field?.name || !field.name.startsWith(prefix) || field.name === basePath) {
                return
              }
              ...
              seen.add(remainder)
              suggestions.push({
                label: field.type ? `${remainder} (${field.type})` : remainder,
                kind:
        ...
        <DialogContent dividers>
          ...
          {cacheExplorerTenant && cacheShapeApi.isFetching && (
            <Stack direction="row" spacing={1} sx={{ alignItems: 'center', mb: 2 }}>
              ...
              <Typography variant="caption">Loading cached collections...</Typography>
            </Stack>
          )}
          <Stack spacing={1.5}>
                  ...
                  <Box sx={{ flex: 1 }}>
                    <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}>
                      ...
                      {cacheType.count != null && (
                        <Chip label={`${cacheType.count} rows`} size="small" variant="outlined" />
                      )}
                    </Stack>
